...
void keyboard_cb(unsigned char c, int x, int y) {
  ...
  case 'c':
    ...
    break;
  case 'T':
	g_app_data->setInteractionMode(IM_COLORTABLE);///missing mode
	break;
  case 't':
  ...
  case ',':
	...
	break;
  case 'r':
	if(g_app_data->getInteractionMode() == IM_COLORTABLE) {
  ...
  case 'R':
	...
	} else if(g_app_data->getInteractionMode() == IM_FRACTAL) {
