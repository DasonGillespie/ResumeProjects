...
void showMenu(MenuData& menu_data, ActionData& action_data) {
	for(long unsigned int i = 0; i < menu_data.getNames().size(); i++) {
		std::string name = menu_data.getNames()[i];
		std::string description = menu_data.getDescription(name);
		action_data.getOS() << name << ") " << description << std::endl;
...
void takeAction(const std::string& choice, MenuData& menu_data, ActionData& action_data) {
	if(choice == "menu") {
		...
		ActionFunctionType command = menu_data.getFunction(choice);
		if(command != 0) {
			command(action_data);
		} else {
